...
def get_parser():
    ...
    parser.add_argument('--config', default='./config/train/msasl_100/train_vsnet.yaml', help='path to the configuration file')
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, index_t, label, index) in enumerate(process):
            ...
            output = self.model(data, index_t, keep_prob)
        ...
        self.lr_scheduler.step(np.mean(loss_value))
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, index_t, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    output = self.model(data, index_t, 1.)
                    loss = self.loss(output, label)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                if epoch + 1 < self.arg.warm_up_epoch:
