...
class Feeder(Dataset):
    def __init__(self, data_path, num_cls, data_type='j', repeat=1):
        ...
        self.left_arm = np.array([1, 3, 5, 22, 23, 24, 25])
        self.right_arm = np.array([2, 4, 26, 43, 44, 45, 46])
        self.finger1 = np.array([7, 8, 9, 10, 11, 12, 13])
        self.finger2 = np.array([14, 15, 16, 17, 18, 19, 20, 21])
        self.finger3 = np.array([28, 29, 30, 31, 32, 33, 34])
        self.finger4 = np.array([35, 36, 37, 38, 39, 40, 41, 42])
    ...
    def __getitem__(self, index):
        ...
        if self.train_val == 'train':
            ...
            # random_shift:
            data_numpy[0, :, :] += random.random() * 10 - 5.0
            data_numpy[1, :, :] += random.random() * 10 - 5.0
...
if __name__ == '__main__':
    ...
    train_feeder_args = {
        'data_path': './data/MSASL/test_200.pkl',
        'num_cls': 200,
        'data_type': 'j',
        'repeat': 1}
    import os
